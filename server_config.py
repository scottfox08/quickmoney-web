...
app = Flask(__name__)
app.secret_key = 'quick_money_v49_history_nitro'
...
try:
    ...
    historial_col = db_mongo['historial_lives'] # Nueva colección para LIVES
...
<style>
    ...
    :root { --gold: #c5a059; --bg: #000; --card: rgba(10, 10, 12, 0.96); --border: #1e1e24; --green: #2ecc71; --red: #ff4757; }
...
def panel():
    ...
    historial = list(historial_col.find(query).sort("_id", -1).limit(50))
        ...
        <div class="main-grid">
            <div>
                ...
                <div class="card"><span class="card-h">👤 PANEL DE USUARIO</span>
                    <p style="font-size:11px; opacity:0.7;">Status: Premium Nitro</p>
                    <p style="font-size:11px;">Telegram: <b style="color:var(--gold)">{u_data.get('telegram','-')}</b></p>
                    <a href="https://t.me/quickmoney_support24" class="btn btn-gold btn-mini" style="text-decoration:none; display:block; text-align:center;">RECARGAR SALDO</a>
                </div>
                '''}
                
                <div class="card"><span class="card-h">📜 HISTORY LIVES (DATABASE)</span>
                    <div style="max-height: 300px; overflow-y: auto;">
                        <table id="db_history">
                            <tr><th>CARD</th><th>FECHA</th></tr>
                            {''.join([f"<tr><td>{h['cc']}</td><td style='opacity:0.5'>{h['fecha']}</td></tr>" for h in historial])}
                        </table>
                    </div>
                </div>
            </div>

            <div class="card"><span class="card-h">🪄 CHECKER / GENERADOR</span>
                <div class="flex-row">
                    <input id="bin_val" placeholder="BIN" value="473702" style="margin-bottom:0; flex:1;">
                ...
                <div style="margin-top:20px;">
                    <span style="color:var(--green); font-size:10px;">LIVES RECIENTES</span>
                    <div class="res-box" id="lives_log" style="height:100px; min-height:100px; margin-top:5px;"></div>
                </div>
                <div style="margin-top:10px;">
                    <div style="display:flex; justify-content:space-between; align-items:center;">
                        <span style="color:var(--red); font-size:10px;">DEAD LOG</span>
                        <button class="btn btn-dark btn-mini" onclick="limpiarDead()">BORRAR</button>
                    </div>
                    <div class="res-box" id="dead_log" style="height:100px; min-height:100px; opacity:0.6; margin-top:5px;"></div>
                </div>
            </div>
        </div>
    </div>
    ...
    async function startChecking() {{
        ...
        for (let cc of lines) {{
            ...
            if(d.status === 'LIVE') {{
                ...
                // Actualizar tabla de historial visualmente
                let table = document.getElementById('db_history');
                let row = table.insertRow(1); row.insertCell(0).innerText = cc; row.insertCell(1).innerText = "RECIENTE";
            }} else {{
...
# --- [ RUTAS DE SOPORTE IGUALES ] ---
@app.route('/')
def login():
    if 'user' in session: return redirect(url_for('panel'))
    return render_template_string(f'<html><head><meta name="viewport" content="width=device-width, initial-scale=1">{CSS}</head><body><canvas id="bg-canvas"></canvas><div style="display:flex;align-items:center;justify-content:center;height:100vh;"><div class="card" style="width:320px; text-align:center; z-index:10;"><h2>⚡️🌩️Quick Money🌩️⚡️</h2><br><form method="POST" action="/auth"><input name="u" placeholder="USUARIO"><input type="password" name="p" placeholder="PASS"><button class="btn btn-gold">INGRESAR</button></form></div></div></body></html>')
