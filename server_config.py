...
app = Flask(__name__)
app.secret_key = 'mairo_profit_king_2026'
...
COSTO_LIVE = 0.35 # Costo por cada tarjeta aprobada
...
<style>
    ...
    .res-box { border-radius: 8px; padding: 10px; font-family: monospace; font-size: 12px; min-height: 60px; margin-top: 10px; }
...
@app.route('/')
def login():
    return render_template_string(f'<html><head>{CSS}</head><body style="display:flex;align-items:center;justify-content:center;height:100vh;"><div class="card" style="width:320px;text-align:center;"><h2>🦁 LOGIN ELITE</h2><form method="POST" action="/auth"><input name="u" placeholder="USUARIO" required><input type="password" name="p" placeholder="PASS" required><button class="btn btn-verify" style="margin-top:10px;">INGRESAR</button></form></div></body></html>')
...
def panel():
    ...
    <div class="container">
        ...
        <div class="card">
            ...
            <form method="POST">
                ...
                <textarea id="gen_area" rows="4" readonly>{gen_res}</textarea>
                <button type="button" class="btn" style="background:#7a632d;color:#ffeb3b" onclick="document.getElementById('check_list').value += document.getElementById('gen_area').value + '\\n'">➕ CARGAR AL CHECKER</button>
            </form>
        </div>

        <div class="card">
            <span class="card-h">🛡️ VALIDACIÓN EN TIEMPO REAL (TARIFADO)</span>
            <textarea id="check_list" rows="6" placeholder="LISTA CC|MM|YY|CVV"></textarea>
            <button class="btn btn-verify" onclick="startChecking()">🚀 INICIAR CHECK ($0.35/LIVE)</button>
            <p style="font-size:9px; color:#555; text-align:center; margin-top:5px;">Las tarjetas se descuentan de la lista conforme se validan.</p>
        </div>

        <div class="card live"><span class="card-h">LIVES ✅</span><div id="lives_log"></div></div>
        <div class="card dead"><span class="card-h">DEAD ❌</span><div id="dead_log"></div></div>

        { f'<a href="/admin" class="btn" style="border:1px solid var(--gold); color:var(--gold); text-decoration:none; display:block; text-align:center;">⚙️ PANEL ADMIN</a>' if u_data['rango'] == 'OWNER' else '' }
    </div>
    ...
    async function startChecking() {{
        ...
        if (!lines[0]) return alert('Ingresa tarjetas');

        while (lines.length > 0) {{
            let currentCC = lines.shift(); // Descontar 1 de la lista
            area.value = lines.join('\\n'); // Actualizar el cuadro de texto

            // Simulación de respuesta del API de Amazon
            let isLive = Math.random() > 0.7; 
            ...
            await new Promise(r => setTimeout(r, 1000)); // Esperar 1 seg por tarjeta
        }}
    }}
    </script>
    </body></html>
...
def admin():
    ...
    return render_template_string(f'<html><head>{CSS}</head><body><div class="container"><div class="card"><h2>⚙️ RECARGAR SALDO</h2><form method="POST"><select name="u_target">{" ".join([f"<option value='{u}'>{u} (${DB['usuarios'][u]['saldo']})</option>" for u in DB["usuarios"]])}</select><input type="number" step="0.01" name="amount" placeholder="Monto $"><button class="btn btn-verify">CARGAR</button></form><br><a href="/panel" style="color:var(--gold)">Volver</a></div></div></body></html>')
