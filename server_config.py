...
app = Flask(__name__)
app.secret_key = 'quick_money_v49_ultimate_nitro'
...
<style>
    ...
    :root { --gold: #c5a059; --bg: #000; --card: rgba(12, 12, 15, 0.95); --border: #1e1e24; --green: #2ecc71; --red: #ff4757; }
    ...
    .container { max-width: 1100px; margin: auto; padding: 20px; position: relative; z-index: 10; }
    .main-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; }
    @media (max-width: 850px) { .main-grid { grid-template-columns: 1fr; } }
    .card { background: var(--card); border: 1px solid var(--border); padding: 20px; margin-bottom: 20px; border-radius: 4px; backdrop-filter: blur(10px); height: fit-content; }
...
def panel():
    ...
    <div class="container">
        ...
        <div style="text-align:center; margin:30px 0; font-size:10px;">
    ...
    let particles = [];
    class Particle {{ constructor() {{ this.x = Math.random() * canvas.width; this.y = Math.random() * canvas.height; this.size = Math.random() * 1.5; this.speedX = Math.random() * 0.4 - 0.2; this.speedY = Math.random() * 0.4 - 0.2; this.color = '#c5a059'; }}
