...
app = Flask(__name__)
app.secret_key = 'quick_money_v49_adaptive_nitro'
...
# --- [ DISEÑO MAESTRO ADAPTATIVO ] ---
CSS = """
<style>
    @import url('https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@300;400;700&display=swap');
    :root { --gold: #c5a059; --bg: #000; --card: rgba(10, 10, 12, 0.95); --border: #1e1e24; --green: #2ecc71; --red: #ff4757; }
    ...
    .container { max-width: 1100px; margin: auto; padding: 15px; position: relative; z-index: 10; }
    
    /* Grid Adaptativo */
    .main-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; }
    @media (max-width: 900px) { .main-grid { grid-template-columns: 1fr; } }

    .card { background: var(--card); border: 1px solid var(--border); padding: 20px; margin-bottom: 20px; border-radius: 4px; backdrop-filter: blur(10px); height: 100%; box-sizing: border-box; }
    ...
    .btn { border: none; padding: 12px; font-weight: bold; cursor: pointer; text-transform: uppercase; font-size: 10px; transition: 0.2s; font-family: inherit; border-radius: 2px; display: inline-block; }
    ...
    .res-box { border-radius: 2px; padding: 10px; font-size: 11px; min-height: 120px; border: 1px solid #1e1e24; background: #030303; overflow-y: auto; max-height: 300px; margin-bottom: 10px; }
...
<script>
    ...
    class Particle {
        constructor() {
            ...
            this.size = Math.random() * 1.5; this.speedX = Math.random() * 0.6 - 0.3;
            this.speedY = Math.random() * 0.6 - 0.3; this.color = '#c5a059';
        }
        update() {
    ...
    function animate() {
        ...
        if(Math.random() > 0.99) { ctx.fillStyle = 'rgba(197, 160, 89, 0.05)'; ctx.fillRect(0, 0, canvas.width, canvas.height); }
    ...
    <div class="container">
        ...
        <div class="main-grid">
            <div>
                {f'''<div class="card" style="border:1px solid var(--gold)"><span class="card-h">👑 ADMIN NITRO CONTROL</span>
                    <form action="/update_sk" method="POST" class="flex-row">
                        <input name="new_sk" placeholder="SK_LIVE_..." value="{sk_val}" style="margin-bottom:0; flex:1; min-width:150px;">
                    ...
                    <form action="/admin_add_saldo" method="POST" class="flex-row">
                        <input name="target_user" placeholder="USER" style="margin-bottom:0; flex:1; min-width:100px;">
                ...
                <div class="card"><span class="card-h">👤 PANEL DE USUARIO</span>
                    <p style="font-size:12px; color:#aaa;">Bienvenido al sistema élite de Quick Money.</p>
                    <div style="border-left:2px solid var(--gold); padding-left:15px; margin:20px 0;">
                        <div style="font-size:10px; color:var(--gold);">TELEGRAM ID:</div>
                        <div style="font-size:13px;">{u_data.get('telegram','No asignado')}</div>
                    </div>
                    <div style="border-left:2px solid var(--gold); padding-left:15px; margin:20px 0;">
                        <div style="font-size:10px; color:var(--gold);">COSTO POR LIVE:</div>
                        <div style="font-size:13px;">$0.15 USD</div>
                    </div>
                    <a href="https://t.me/quickmoney_support24" target="_blank" class="btn btn-gold" style="text-align:center; text-decoration:none;">SOLICITAR CARGA DE SALDO</a>
                </div>
                '''}
            </div>
            ...
            <div class="card"><span class="card-h">🪄 GENERADOR / CHECKER</span>
                ...
                <textarea id="check_list" rows="7" placeholder="CC|MM|YY|CVV"></textarea>
        ...
        <div class="main-grid" style="margin-top:20px;">
            <div>
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:5px;">
                    ...
                    <button class="btn btn-dark btn-mini" onclick="descargarLives()">📥 DESCARGAR</button>
                </div>
            ...
            <div>
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:5px;">
                    ...
                    <button class="btn btn-dark btn-mini" onclick="limpiarDead()">🗑️ BORRAR DEAD</button>
                </div>
        ...
        <div style="text-align:center; margin-top:30px; font-size:10px;">
            <a href="https://t.me/quickmoney_support24" style="color:var(--gold); margin:0 10px;">SOPORTE</a> | 
            <a href="https://t.me/+GUlp9rhO0_k1ZWYx" style="color:var(--gold); margin:0 10px;">GRUPO</a> | 
            <a href="/logout" style="color:#666; margin:0 10px;">CERRAR SESIÓN</a>
        </div>
    </div>
    {JS_SCRIPT}
    </body></html>
...
def register():
    ...
    return render_template_string(f'<html><head><meta name="viewport" content="width=device-width, initial-scale=1">{CSS}</head><body><canvas id="bg-canvas"></canvas><div class="container" style="max-width:400px; margin-top:100px;"><div class="card" style="text-align:center;"><h2>REGISTRO</h2><form method="POST"><input name="u" placeholder="USUARIO"><input type="password" name="p" placeholder="PASS"><input name="t" placeholder="TELEGRAM @ID"><button class="btn btn-gold">REGISTRARSE</button></form></div></div>{JS_SCRIPT}</body></html>')
