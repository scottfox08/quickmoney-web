...
app = Flask(__name__)
app.secret_key = 'quick_money_v28_final_boss_blindado'
...
# --- MOTOR DE DETECCIÓN REALISTA (V27 + V28) ---
...
<style>
    ...
    .auth-card { background: var(--card); border: 1px solid var(--border); border-radius: 4px; padding: 35px; width: 340px; text-align: center; border-top: 3px solid var(--gold); backdrop-filter: blur(10px); }
    ...
    .qm-title { font-size: 20px; font-weight: 700; color: #fff; margin-bottom: 30px; font-family:'Montserrat'; letter-spacing: -1px; display: flex; align-items: center; justify-content: center; gap: 8px; }
    ...
    .live-info { font-size: 10px; color: var(--gold); text-transform: uppercase; margin-top: 3px; display: block; opacity: 0.8; }
...
<script>
    ...
    function h() { p.push(new Particle()); for(let i=0;i<p.length;i++){ p[i].u(); p[i].d(); if(p[i].s<=0.3){p.splice(i,1); i--;} } }
    function a() { ctx.clearRect(0,0,canvas.width,canvas.height); h(); requestAnimationFrame(a); } a();
...
def login():
    ...
    <div class="auth-card">
        <div class="houdini-frame"></div><div class="qm-title">⚡️ 🌩️ QUICK MONEY 🌩️ ⚡️</div>
        <form method="POST" action="/auth"><input name="u" placeholder="USUARIO" required><input type="password" name="p" placeholder="PASS" required><button class="btn btn-gold">[ INGRESAR ]</button></form>
        <div style="margin-top:20px; font-size:11px;">¿NUEVO? <a href="/register" style="color:var(--gold); font-weight:bold;">REGÍSTRATE</a></div>
    </div>{CANVAS_SCRIPT}</body></html>
    """)
...
def register():
    ...
    if request.method == 'POST':
        ...
        elif u in db["usuarios"]: error = "OCUPADO"
        else:
...
def panel():
    ...
    <div class="container">
        ...
        <div class="card"><span class="card-h">🛡️ GATE AMAZON V21</span>
            <input id="amazon_cookie" placeholder="AMAZON COOKIE..."><textarea id="check_list" rows="6" placeholder="LISTA CC|MM|YY|CVV"></textarea>
            ...
            <div style="display:flex; gap:10px; margin-top:10px;"><button class="btn btn-dark" style="flex:1" onclick="document.getElementById('check_list').value = ''">🗑️ LIMPIAR</button>
                <button class="btn btn-dark" style="flex:1; color:var(--green)" onclick="downloadLives()">📥 DESCARGAR</button></div>
        </div>
    ...
    function downloadLives() {{ const blob = new Blob([livesArray.join('\\n')], {{ type: 'text/plain' }}); const url = window.URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'lives.txt'; a.click(); }}
    </script></body></html>
    """)
...
def validar():
    ...
    if random.random() > 0.8: # Simulación de Live
