...
def check_gate_pro(cc):
    try:
        ...
        if response.status_code == 200:
            ...
            try:
                msg = response.json().get('error', {}).get('message', 'DECLINED').upper()
...
def panel():
    ...
    <body><canvas id="bg-canvas"></canvas><div class="container">
        ...
        <div class="card"><span class="card-h">🪄 GENERADOR INTELIGENTE</span>
            ...
            <div style="display:flex; gap:10px;">
                ...
                <button class="btn btn-danger btn-mini" onclick="document.getElementById('gen_area').value = ''">🗑️ BORRAR</button>
            </div>
        </div>
    ...
    function generarCC() {{
        ...
        let bin = input[0];
        let mes = input[1] || ("0"+(Math.floor(Math.random()*12)+1)).slice(-2);
        let ano = input[2] || (2025+Math.floor(Math.random()*6));
        let c = []; 
        for(let i=0;i<10;i++) {{
            let r = bin; while(r.length<16) r+=Math.floor(Math.random()*10);
            c.push(r+"|"+mes+"|"+ano+"|"+Math.floor(Math.random()*899+100));
        }} 
